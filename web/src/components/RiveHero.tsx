...
export default function RiveHero({title, link}: RiveButtonProps) {
  ...
  return (
    <div
      className="relative  rive-canvas-container w-full h-full"
      style={{ width: "100%", height: "100%" }}
      ref={setContainerRef}
      onMouseMove={onMouseMove}
    >
      <canvas
        className=" rive-canvas block relative w-full h-full max-h-screen max-w-screen align-top"
        ref={setCanvasRef}
        style={{ width: "100%", height: "100%" }}
        aria-label="Hero element for the Explore page; an interactive graphic showing planets thru a spacesuit visor"
      ></canvas>
      <RiveButton title={title} link={link}/>
    </div>
  );
}